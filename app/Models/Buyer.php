...
final class Buyer
{
    ...
    public static function findOrCreate(string $fullName, string $nic, string $mobile, string $email = ''): int
    {
        ...
        if ($nic !== '') {
            ...
            $existing = $stmt->fetch();
            if ($existing) {
                $update = $db->prepare('UPDATE buyers SET full_name = :name, mobile = COALESCE(NULLIF(:mobile, \'\'), mobile), email = :email WHERE id = :id');
    ...
    {
        $stmt = Database::connection()->prepare('UPDATE buyers SET mobile = :mobile WHERE id = :id');
